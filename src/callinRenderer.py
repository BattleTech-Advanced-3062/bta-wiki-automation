import os
import sys
import json
from pprint import pp
import genUtilities
import callinParser
from settings import *

template = environment.get_template("callin.tpl")

session, csrf_token = genUtilities.create_wiki_session()

def render_callin_table(data):
    #context = {
        #"callins": callins
    #}
    if "GITHUB_ACTIONS" in os.environ or "LOCAL_OVERRIDE" in os.environ:
        # Wiki page writing
        print("Posting to the wiki")
        page_title = "Test Beacons"
        genUtilities.post_to_wiki(session, csrf_token, page_title, template.render(**data))
    else:
        bulk_filename = "bulk_callins.wiki"
        #pp(context)
        print("Writing locally")
        with open(bulk_filename, mode="w", encoding="utf-8") as callins:
                callins.write(template.render(**data))

if __name__ == "__main__":
    result = callinParser.process_all_callins()
    render_callin_table(result)